...
**方法 C：桌面快捷方式（一次性创建）**
- 右键桌面 → 新建 → 快捷方式
- 目标: `"C:\Program Files\Google\Chrome\Application\chrome.exe" --remote-debugging-port=9222 --user-data-dir=[CHROME_USER_DATA_DIR]`
- 命名: "Chrome CDP"

### 3. 首次使用：同步登录态

首次用 `--user-data-dir=[CHROME_USER_DATA_DIR]` 启动时是空白 profile：
...
| `--user-data-dir=[CHROME_USER_DATA_DIR]` | 使用固定路径 profile（首次需登录，后续保留） |
| `--user-data-dir=<随机临时目录>` | ❌ 每次空白，登录态丢失 |

**建议用固定路径** `[CHROME_USER_DATA_DIR]`，好处是隔离了调试浏览器和日常浏览器的 profile，避免调试操作污染日常浏览数据。
...
但如果用的是新 profile（`--user-data-dir=[CHROME_USER_DATA_DIR]`），需先同步 Google 密码。
首次同步后，后续登录自动填充。
